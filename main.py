...
# ── Import Ask AI tab ───────────────────────────────────────────
from ask_ai_tab import render_ask_ai_tab, capture_chart
...
def _show(fig, label=""):
    """Helper: st.pyplot + capture_chart."""
    st.pyplot(fig)
    capture_chart(fig, label)
    plt.close(fig)
...
def plot_figure1_pairwise(df):
    ...
    fig.suptitle("Figure 1 · Pairwise Relationships", fontsize=14, fontweight="bold", color=C_BLUE, y=1.01)
    plt.tight_layout()
    return fig
...
def plot_figure3_comparison(y_uni, y_pred_uni, r2_uni,
    ...
    fig.suptitle("Figure 3 · Predicted vs Actual Values", fontsize=14, fontweight="bold", color=C_BLUE, y=1.02)
    plt.tight_layout()
    return fig
...
with tabs[0]:
    ...
    _show(fig1, "Fig1 · Pairwise Relationships")
    ...
    with col_l:
        ...
        _show(fig2, "Fig2 · Correlation Matrix")
    ...
    with col_a:
        ...
        ax.set_xlabel("Price (Millions)"); ax.set_ylabel("Frequency")
        ax.set_title("Dengan Outlier", color=C_BLUE); ax.grid(True, alpha=0.25)
        plt.tight_layout()
        _show(fig_d1, "Distribusi Harga Dengan Outlier")
    with col_b:
        fig_d2, ax = plt.subplots(figsize=(7, 3.5))
        ax.hist(df_no["price"] / 1e6, bins=40, color=C_GREEN, edgecolor="#0E1117", alpha=0.85)
        ax.set_xlabel("Price (Millions)"); ax.set_ylabel("Frequency")
        ax.set_title("Tanpa Outlier", color=C_GREEN); ax.grid(True, alpha=0.25)
        plt.tight_layout()
        _show(fig_d2, "Distribusi Harga Tanpa Outlier")
...
with tabs[5]:
    ...
    comp_df = pd.DataFrame({
        "Model": ["📈 Univariate (area)", "📉 Multivariate + Outlier",
                  "📉 Multivariate − Outlier", "🌲 XGBoost"],
    ...
    _show(fig3, "Fig3 · All Models Comparison")
    st.divider()
    col_a, col_b = st.columns(2)
    with col_a:
    ...
    with col_b:
        ...
        - **Multi − Outlier** ({r2_no*100:.2f}%): Skor turun tapi visual lebih bersih
        - **XGBoost** ({r2_xgb*100:.2f}%): Superior untuk non-linear
        """)
    st.markdown("""
    ##### 📚 Limitasi & Future Work (Section 4)
    **Limitasi:** Asumsi linearitas, sensitif outlier, homoscedasticity, normalitas error  
    **Future:** Polynomial/log regression, robust regression, Random Forest, XGBoost, deep learning
    """)
...
with tabs[6]:
    ...
    if submit_btn:
        ...
        input_raw = np.array([[
            ...
            yn(mainroad), yn(guestroom), yn(basement), yn(hotwater), yn(aircon),
            parking, yn(prefarea), furnish_map_input[furnishing]
        ]], dtype=float)

        # Univariate
        input_raw_uni = np.array([[area]], dtype=float)
        rng_uni = maxs_uni - mins_uni; rng_uni[rng_uni == 0] = 1
        input_scaled_uni = (input_raw_uni - mins_uni) / rng_uni
        price_pred_uni = float((np.hstack([[[1]], input_scaled_uni]) @ beta_uni)[0])

        # Multi + Outlier
        rng_multi = maxs_multi - mins_multi; rng_multi[rng_multi == 0] = 1
        input_scaled_multi = (input_raw - mins_multi) / rng_multi
        price_pred_multi = float((np.hstack([[[1]], input_scaled_multi]) @ beta_multi)[0])

        # Multi - Outlier
        rng_no = maxs_no - mins_no; rng_no[rng_no == 0] = 1
        input_scaled_no = (input_raw - mins_no) / rng_no
        price_pred_no = float((np.hstack([[[1]], input_scaled_no]) @ beta_no)[0])

        # XGBoost
        rng_xgb = maxs_xgb - mins_xgb; rng_xgb[rng_xgb == 0] = 1
        input_scaled_xgb = (input_raw - mins_xgb) / rng_xgb
        ...
        preds = [
            ("📈 Univariate", price_pred_uni, r2_uni, mse_uni),
            ("📉 Multi + Outlier", price_pred_multi, r2_multi, mse_multi),
            ("📉 Multi − Outlier", price_pred_no, r2_no, mse_no),
            ("🌲 XGBoost", price_pred_xgb, r2_xgb, mse_xgb),
        ]
        for col, (label, price, r2, m) in zip(res_cols, preds):
            with col:
                st.markdown(f"##### {label}")
                if price > 0:
                    ...
                    st.metric("R²", f"{r2*100:.2f}%")
                    rmse_val = np.sqrt(m)
                    st.metric("RMSE", f"{rmse_val:,.0f}")
                    st.caption(f"Range: {max(0, price-rmse_val):,.0f} — {price+rmse_val:,.0f}")
                else:
